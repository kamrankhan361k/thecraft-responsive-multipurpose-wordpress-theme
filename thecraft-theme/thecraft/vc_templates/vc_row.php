 ...
 */
$el_class = $img_position = $halfrow_image = $img_columns = $img_halfrow = $full_height = $full_width = $equal_height = $flex_row = $columns_placement = $content_placement = $parallax = $parallax_image = $css = $el_id = $video_bg = $video_bg_url = $video_bg_parallax = $fullwidth = $css_animation = $image_video = $row_middle_content = $row_content_position = '';
$disable_element = '';
$output = $after_output = '';
...
if ( 'yes' === $row_middle_content ) $css_classes[] = 'row-middle-content';
...
$thecraft_wrap_classes = 'vc-custom-col-spacing clearfix';
if ( ! empty( $column_spacing ) ) {
	$thecraft_wrap_classes .= ' vc-col-spacing-'. $column_spacing;
}

$output .= '<div class="'. $thecraft_wrap_classes .'">';

$output .= '<section '. implode( ' ', $wrapper_attributes ) .'>';

$video_icon = '';
if ( $image_video )
	$video_icon = '<div class="wprt-video-icon clearfix white"><a class="icon-wrap popup-video" href="'. $image_video .'">play<span class="circle"></span><span class="circle-2"></span></a></div>';
...
if ($fullwidth == 'yes') {
	...
	$output .= '<div class="wprt-container">';
	$output .= '<div class="row-inner clearfix">' .wpb_js_remove_wpautop( $content ) .'</div>';
	$output .= '</div>';
}
...
if ( $img_halfrow == 'parallax' ) {
	...
	$output .= '<div class="image-container img-parallax '. implode( ' ', $img_asiderow_attributes ) .'" style="top:'. $img_offset_top .'"><div class="inner" style="right:'. $img_offset_right .'"><img src="'. esc_url( $halfrow_image_src ) .'" data-parallax=\'{"x" : '. $px .', "y" : '. $py .'}\' alt="'. esc_attr__( 'Image', 'thecraft' ) .'">'. $video_icon .'</div></div>';
}
