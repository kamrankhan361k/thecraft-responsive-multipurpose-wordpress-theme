...
function wprt_header_search() {
	...
	<div id="header-search">
    	...
    	<form role="search" method="get" class="header-search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>">
			<label class="screen-reader-text"><?php esc_html_e('Search for:', 'thecraft'); ?></label>
			<input type="text" value="<?php the_search_query(); ?>" name="s" class="header-search-field" placeholder="<?php esc_attr_e('SEARCH...', 'thecraft'); ?>" />
			<button type="submit" class="header-search-submit" title="<?php esc_attr_e('Search', 'thecraft'); ?>">
				<?php esc_html_e('Search', 'thecraft'); ?>
			</button>
...
function wprt_body_classes() {
	...
	// Get top-bar style
	$top_bar_style = wprt_get_mod( 'top_bar_site_style', 'style-1' );
	if ( is_page() && wprt_metabox('top_bar_style') )
		$top_bar_style = wprt_metabox('top_bar_style');
	...
	$cur_menu_style = wprt_get_mod( 'menu_link_current', 'cur-menu-1' );
	if ( $header_style == 'style-2' ) $cur_menu_style = wprt_get_mod( 'menu_two_link_current', 'cur-menu-1' );
	if ( $header_style == 'style-3' ) $cur_menu_style = wprt_get_mod( 'menu_three_link_current', 'cur-menu-1' );
	if ( $header_style == 'style-4' ) $cur_menu_style = wprt_get_mod( 'menu_four_link_current', 'cur-menu-1' );
	$classes[] = $cur_menu_style;

	if ( is_page() ) $classes[] = 'is-page';

	if ( ( is_page() && wprt_metabox('hide_padding_content') )
		|| ( is_singular('project') && wprt_metabox('hide_padding_content') )
		)
...
function wprt_content_length() {
	$length = wprt_get_mod( 'blog_excerpt_length', '108' );

	if ( is_page_template( 'templates/page-blog-grid.php' ) )
		$length = wprt_get_mod( 'blog_excerpt_length_grid', '35' );

	if ( is_page_template( 'templates/page-blog-list.php' ) )
		$length = wprt_get_mod( 'blog_excerpt_length_list', '75' );
	...
	foreach ( $meta_item as $item ) :
		if ( 'author' == $item ) { 
			printf( '<span class="post-by-author item"><span class="inner">By <a href="%s" title="%s" rel="author">%s</a></span></span>',
				...
				esc_attr( sprintf( esc_html__( 'View all posts by %s', 'thecraft' ), get_the_author() ) ),
				get_the_author()
				);
		}
		elseif ( 'date' == $item ) {
		...
		elseif ( 'comments' == $item ) {
			if ( comments_open() || get_comments_number() ) {
				...
				comments_popup_link( esc_html__( '0 comment', 'thecraft' ), esc_html__( '1 Comment', 'thecraft' ), esc_html__( '% Comments', 'thecraft' ) );
				echo '</span></span>';
			}
		}
...
function wprt_topbar_social_options() {
	return apply_filters ( 'wprt_topbar_social_options', array(
		'facebook' => array(
			'label' => esc_html__( 'Facebook', 'thecraft' ),
			'icon_class' => 'craft-facebook',
		),
		'twitter' => array(
			'label' => esc_html__( 'Twitter', 'thecraft' ),
			'icon_class' => 'craft-twitter',
		),
		'youtube' => array(
			'label' => esc_html__( 'Youtube', 'thecraft' ),
			'icon_class' => 'craft-youtube',
		),
		'vimeo' => array(
			'label' => esc_html__( 'Vimeo', 'thecraft' ),
			'icon_class' => 'craft-vimeo',
		),
		'tumblr'  => array(
			'label' => esc_html__( 'Tumblr', 'thecraft' ),
			'icon_class' => 'craft-tumblr',
		),
		'pinterest'  => array(
			'label' => esc_html__( 'Pinterest', 'thecraft' ),
			'icon_class' => 'craft-pinterest',
		),
		'linkedin'  => array(
			'label' => esc_html__( 'LinkedIn', 'thecraft' ),
			'icon_class' => 'craft-linkedin',
		),
		'instagram'  => array(
			'label' => esc_html__( 'Instagram', 'thecraft' ),
			'icon_class' => 'craft-instagram',
		),
		'skype' => array(
			'label' => esc_html__( 'Github', 'thecraft' ),
			'icon_class' => 'craft-github',
		),
		'Apple' => array(
			'label' => esc_html__( 'Apple', 'thecraft' ),
			'icon_class' => 'craft-apple',
		),
		'android' => array(
			'label' => esc_html__( 'Android', 'thecraft' ),
			'icon_class' => 'craft-android',
		),
		'behance'  => array(
			'label' => esc_html__( 'Behance', 'thecraft' ),
			'icon_class' => 'craft-behance',
		),
		'dribbble' => array(
			'label' => esc_html__( 'Dribbble', 'thecraft' ),
			'icon_class' => 'craft-dribbble',
		),
		'flickr'  => array(
			'label' => esc_html__( 'Flickr', 'thecraft' ),
			'icon_class' => 'craft-flickr',
		),

		'paypal'  => array(
			'label' => esc_html__( 'Paypal', 'thecraft' ),
			'icon_class' => 'craft-paypal',
		),
		'soundcloud'  => array(
			'label' => esc_html__( 'Soundcloud', 'thecraft' ),
			'icon_class' => 'craft-soundcloud',
		),
		'spotify'  => array(
			'label' => esc_html__( 'Spotify', 'thecraft' ),
			'icon_class' => 'craft-spotify',
		),
	) );
}
