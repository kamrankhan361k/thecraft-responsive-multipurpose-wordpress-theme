...
function wprt_google_fonts_array() {
...
function wprt_enqueue_google_font( $font) {

	// Get list of all Google Fonts
	$google_fonts = wprt_google_fonts_array();
	...
	// Subset
	$subset = wprt_get_mod( 'google_font_subsets', 'latin' );
	$subset = $subset ? $subset : 'latin';
	$subset = '&amp;subset='. $subset;

	// Weights
	$weights = array( '100', '200', '300', '400', '500', '600', '700', '800', '900' );
	$weights = apply_filters( 'wprt_google_font_enqueue_weights', $weights, $font );
	$italics = apply_filters( 'wprt_google_font_enqueue_italics', true );

	// Main URL
	$url = wprt_get_google_fonts_url() .'/css?family='. str_replace(' ', '%20', $font ) .':';
	...
	// Enqueue style
	wp_enqueue_style( 'wprt-google-font-'. $handle, $url, false, false, 'all' );
}