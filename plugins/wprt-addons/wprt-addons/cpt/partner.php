...
function register_partner_post_type() {
    ...
    $labels = array(
        'name'               => esc_html__( 'Partners', 'thecraft' ),
        'singular_name'      => esc_html__( 'Partner Item', 'thecraft' ),
...
function partner_updated_messages( $messages ) {
    ...
    $messages['partner'] = array(
        ...
        1  => esc_html__( 'Partner updated.', 'thecraft' ),
        2  => esc_html__( 'Custom field updated.', 'thecraft' ),
        3  => esc_html__( 'Custom field deleted.', 'thecraft' ),
        4  => esc_html__( 'Partner updated.', 'thecraft' ),
        5  => isset( $_GET['revision'] ) ? sprintf( esc_html__( 'Partner restored to revision from %s', 'thecraft' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
        6  => esc_html__( 'Partner published.', 'thecraft' ),
        7  => esc_html__( 'Partner saved.', 'thecraft' ),
        8  => esc_html__( 'Partner submitted.', 'thecraft' ),
        9  => sprintf(
            esc_html__( 'Partner scheduled for: <strong>%1$s</strong>.', 'thecraft' ),
            date_i18n( esc_html__( 'M j, Y @ G:i', 'thecraft' ), strtotime( $post->post_date ) )
        ),
        10 => esc_html__( 'Partner draft updated.', 'thecraft' )
    );
    return $messages;
}
...
function register_partner_taxonomy() {
    ...
    $labels = array(
        'name'                       => esc_html__( 'Partner Categories', 'thecraft' ),
