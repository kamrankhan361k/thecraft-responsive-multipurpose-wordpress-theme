...
function register_project_post_type() {
    ...
    $labels = array(
        'name'               => esc_html__( 'Projects', 'thecraft' ),
        'singular_name'      => esc_html__( 'Project Item', 'thecraft' ),
...
function project_updated_messages( $messages ) {
    ...
    $messages['project'] = array(
        ...
        1  => esc_html__( 'Project updated.', 'thecraft' ),
        2  => esc_html__( 'Custom field updated.', 'thecraft' ),
        3  => esc_html__( 'Custom field deleted.', 'thecraft' ),
        4  => esc_html__( 'Project updated.', 'thecraft' ),
        5  => isset( $_GET['revision'] ) ? sprintf( esc_html__( 'Project restored to revision from %s', 'thecraft' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
        6  => esc_html__( 'Project published.', 'thecraft' ),
        7  => esc_html__( 'Project saved.', 'thecraft' ),
        8  => esc_html__( 'Project submitted.', 'thecraft' ),
        9  => sprintf(
            esc_html__( 'Project scheduled for: <strong>%1$s</strong>.', 'thecraft' ),
            date_i18n( esc_html__( 'M j, Y @ G:i', 'thecraft' ), strtotime( $post->post_date ) )
        ),
        10 => esc_html__( 'Project draft updated.', 'thecraft' )
    );
    return $messages;
}
...
function register_project_taxonomy() {
    ...
    $labels = array(
        'name'                       => esc_html__( 'Project Categories', 'thecraft' ),
